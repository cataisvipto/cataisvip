...
### 2026-09-03：CF Pages 部署失败，站点停在旧版本（非数据问题）
- **现象**：Aaron 截图 CF Pages Workers 控制台，`main` 分支 `82e3ecc`/`78f695b` 与 `pages-data` 分支 `3c59978` 三条部署红点❗（"No deployment available"），cataito.com 仍跑 17 小时前 `7614c17` 的旧版本，排名页本身数据正常。
- **定位**：①GitHub Actions 的 `cloudflare-deploy.yml` 在这两个 main 提交上**根本没触发**（最近一次成功仍是 `7614c17`）；②截图红点是 **Cloudflare 自带的"自动 GitHub 部署"通道**——它和 GitHub Actions 是两条独立部署通道；③本地对 `82e3ecc` 跑 `npm run build` 完全通过，代码没问题。
- **根因**：`package-lock.json` 与 `package.json` 依赖声明不同步（`picomatch@4.0.7` vs lock 里的 `2.3.2`）。CF Pages 自动部署走 `npm ci` 严格模式，lock 不同步即 EUSAGE 报错部署失败；而 GitHub Actions 用 `npm install --legacy-peer-deps` 会静默容忍，Action 那边看不出问题——**两条通道对 lock 的宽容度不同，CF 更严**。
- **修复（9e28c65→e804bdc）**：①本地 `rm -rf node_modules && npm install --lockfile-only` 只重算 lock（不动 package.json，顺带解 6 个漏洞）；②`npm ci` + `npm run build` 双验证通过；③commit + push（先 `git pull --rebase` 到远端最新自动刷新提交之上，避免 non-fast-forward）；④CI + Cloudflare Pages Deploy + Vercel Smoke Test 三条 workflow 全绿，`curl` `cataito.com/en` 与 `/en/tool/chatgpt` 双 200。
- **关键教训**：①**改依赖后必须 `npm install --lockfile-only` 再 `npm ci` 验证**，不能只靠本地 `npm run build`（它不重算 lock，也盖不住 CF 的严格校验）；②CF Pages 自动部署与 GitHub Actions 是两条独立通道，前者用 `npm ci` 更严，lock 漂移只会让 CF 红点、Actions 掩盖——**以 CF 的严格结果为准**；③依赖类修复 push 前若远端已有自动刷新提交，需 `git pull --rebase` 到远端 HEAD 之上再 push。详见 skill `solutions.md` #14。
- **后续建议**：统一部署入口，关掉 CF 那侧"自动 GitHub 部署"开关，只保留 GitHub Actions 的 `cloudflare-deploy.yml` 一条路（Aaron 去 CF Dashboard Pages 项目页关 `Automatic GitHub deployments`，我没 CF token 远程关不了）。
